...
def sign_up():
    ...
    userName.set('')
    e3 = tk.Entry(window2,textvariable=userName,show=None,font=('Arial',14),width=15)
    e3.place(x=120,y=40)
    
    tip1 = tk.Label(window2,text=f'△ 账号长度为{least}-{length}位字符,仅含字母数字,不含特殊符号',fg='grey',font=('Arial',8))
    tip1.place(x=90,y=64)

    P_w_length = 20
    P_w_least = 6

    tip2 = tk.Label(window2,text=f'△ 密码长度为{P_w_least}-{P_w_length}位字符')
    
    P_w = tk.StringVar(window2,value='')
    e4 = tk.Entry(window2,textvariable=P_w,show='*',font=('Arial',14),width=15)
    e4.place(x=120,y=80)

    P_w2 = tk.StringVar(window2,value='')
    e5 = tk.Entry(window2,textvariable=P_w2,show='*',font=('Arial',14),width=15)
    ...
    tk.Button(window2,text='确定',command=register,font=('Arial',14),width=15,bg='grey').place(x=120,y=160)
    window2.mainloop()
tk.Button(text='登录',command=login,width=9,bg='grey',font=('Arial',14)).place(x=140,y=140)
tk.Button(text='注册',command=sign_up,width=9,font=('Arial',14)).place(x=250,y=140)
window.mainloop()
